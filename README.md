...
### 提供元データの既知の不整合（版 `20260707`）

属性も元データのまま通しているため、**提供元 GeoJSON に混入している市区町村属性の誤りもそのまま残ります**。
版 `20260707` で確認できているものは次の 3 件です（詳細は [#25](https://github.com/shiwaku/mlit-urban-planning-converter/issues/25)）。

| テーマ | 提供元ファイル | 地物数 | 内容 |
| --- | --- | --- | --- |
| `youto` 用途地域 | `20_長野県/20383_箕輪町/20383_youto.geojson` | 474 | うち 458 件は**新潟市の用途地域の複製**で、属性だけ `長野県 / 20383 / 箕輪町` に書き換わっている。箕輪町の実データは 16 件のみ |
| `kouen` 都市計画公園 | `05_秋田県/05203_横手市/05203_kouen.geojson` | 73 | うち 44 件は**福島県会津若松市周辺の公園**（例: 蒲生氏郷記念公園）。横手市の実データは 29 件 |
| `kouen` 都市計画公園 | `47_沖縄県/47211_沖縄市/47211_kouen.geojson` | 115 | うち 25 件は**大分県別府市周辺の公園**（例: いしくぶ公園）。沖縄市の実データは 90 件 |

- `youto` の 1 件目は複製なので、新潟市の用途地域が**二重に描画され**、ビューアでクリックすると上に載った「箕輪町」側が拾われます。新潟市側のデータ自体は正しく入っています。
- `kouen` の 2 件は複製ではありません。誤配置された公園は本来の自治体（会津若松市・別府市など）のファイルには入っておらず、**そちらでは欠落**しています。
- 検出は「同一 `Citycode` 内でジオメトリ中心が中央値から 0.5° 以上離れる地物」「市区町村の代表点が所属都道府県の代表点から 1.5° 以上離れるもの」を全 26 テーマに当てたもの。
  **隣接自治体との取り違えのような小さいズレは検出できていません**。

集計で自治体別に切る場合は、少なくとも上記 3 ファイル分を除外・補正するか、`Citycode` ではなく行政界ポリゴンとの空間結合で判定してください。
...
- 初期表示は**全26レイヤ ON**（[`data/styles.json`](data/styles.json) の
  `defaultVisible: "all"`）。日本全体を表示したままだと26枚すべてを描くので重くなります。
  使う範囲に拡大するか、レイヤパネルで不要なレイヤのチェックを外してください
  （テーマコードの配列を書けば、その分だけ ON にした `.qlr` / `.qgz` を作れます）
...
- データ: 本パイプラインが取得・変換するデータの著作権・利用条件は、出典である国土交通省 都市局「都市計画決定GISデータ」に従います。
  利用・再配布の際は出典を明記し、提供元の利用規約を確認してください。
  なお本データは参考情報であり、内容の正確性・最新性は保証されません。
